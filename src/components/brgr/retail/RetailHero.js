...
export default function RetailHero({ prop, states, styles: componentStyles, themeColors }) {
  ...
  const fallbackImage = resolveRetailImage(
    propValue(prop, 'backgroundImage', '') ||
  ...
  const slideInterval =
    Number.isFinite(configuredInterval) && configuredInterval > 0 ? configuredInterval : 5;
    ...
    <Box
      ...
      sx={{
        ...
        background: styleValue(
          styles,
          'HeroCarouselDisplayedImageBackgroundColor',
    ...
    >
      <Box
        sx={{
          position: 'relative',
          width: '100%',
          minHeight: styleLength(styles, 'HeroCarouselDisplayedImageHeight', { xs: 330, md: 475 }),
          overflow: 'hidden',
          background: styleValue(
            styles,
            'HeroCarouselDisplayedImageBackgroundColor',
